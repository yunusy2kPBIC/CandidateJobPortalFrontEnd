...
import { api, type JobList } from '../services/api'
        ...
        <section className="jobs-main">
          <form className="panel search-panel" onSubmit={submit}>
            ...
            <div className="search-fields">
              ...
              <label><span><Globe2 size={18} />Country</span><select value={filters.country} onChange={(event) => update('country', event.target.value)}><option value="">All countries</option>{data.filters.countries.map((item) => <option key={item}>{item}</option>)}</select></label>
              <label><span><MapPin size={18} />City</span><select value={filters.city} onChange={(event) => update('city', event.target.value)}><option value="">All cities</option>{data.filters.cities.map((item) => <option key={item}>{item}</option>)}</select></label>
              <label><span><Building2 size={18} />Division</span><select value={filters.division} onChange={(event) => update('division', event.target.value)}><option value="">All divisions</option>{data.filters.divisions.map((item) => <option key={item}>{item}</option>)}</select></label>
              <label><span><BriefcaseBusiness size={18} />Job function</span><select value={filters.job_function} onChange={(event) => update('job_function', event.target.value)}><option value="">All job functions</option>{data.filters.job_functions.map((item) => <option key={item}>{item}</option>)}</select></label>
              <label><span><BarChart3 size={18} />Career level</span><select value={filters.career_level} onChange={(event) => update('career_level', event.target.value)}><option value="">All career levels</option>{data.filters.career_levels.map((item) => <option key={item}>{item}</option>)}</select></label>
            </div>
          ...
          {loading ? <div className="loading-stack">{[1, 2, 3].map((item) => <div className="skeleton job-card" key={item} />)}</div> : data.items.length ? <div className="job-list">{data.items.map((job) => <JobCard key={job.id} job={job} />)}</div> : <EmptyState title="No matching roles" description="Try broadening your filters or clearing the search." />}
        </section>
